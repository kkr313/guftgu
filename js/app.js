...
document.addEventListener('DOMContentLoaded', async () => {
  // Wait for all page partials to be injected into #app
  if (window._pagesReady) await window._pagesReady;
  ...
  // Set real stats immediately — replaces any hardcoded HTML placeholders
...
// ─────────────────────────────────────────────────────────────────
// CALL HISTORY — self-contained so no dependency on home.js
// ─────────────────────────────────────────────────────────────────
...
function saveCallToHistory(avatar, name, mood, duration, type) {
  ...
  updateProfileStats();
  if (typeof renderRecentCalls === 'function') renderRecentCalls();
}
...
function updateProfileStats() {
  const history = _getCallHistory();

  // Case-insensitive so 'Outgoing' and 'outgoing' both count
  const callCount = history.filter(c => {
    const t = (c.type || '').toLowerCase();
    return t === 'incoming' || t === 'outgoing';
  }).length;

  const chatCount = new Set(history.map(c => c.name).filter(Boolean)).size;

  let friendCount = 0;
  try {
    const raw = localStorage.getItem('guftgu_friends');
    friendCount = raw ? JSON.parse(raw).length : 0;
  } catch(e) {}

  const sc = document.getElementById('statChats');
  const sv = document.getElementById('statCalls');
  const sf = document.getElementById('statFriends');
  if (sc) sc.textContent = chatCount;
  if (sv) sv.textContent = callCount;
  if (sf) sf.textContent = friendCount;
}
