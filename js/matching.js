...
function _startCoordinatedCall(matchId, role) {
  ...
  if (role === 'caller') {
    // --- CALLER: create WebRTC room, publish roomId ---
    const roomId = (typeof genRoomId === 'function') ? genRoomId() :
      Math.random().toString(36).substr(2,6).toUpperCase();

    if (typeof createRoom === 'function') {
      createRoom(roomId).then(() => {
        // Write roomId into match so callee can find and join it
        fbDb.ref('matches/' + matchId + '/webrtcRoomId').set(roomId);
      }).catch(err => {
        showToast('Mic error: ' + (err.message || 'Check permissions'));
      });
    }

  } else {
    // --- CALLEE: wait for caller to publish roomId, then join ---
